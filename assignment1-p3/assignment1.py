...
images_sobelH = [cv2.filter2D(image , -1 , sobelHKernel) for image in
                 images]

images_sobelV = [cv2.filter2D(image , -1 , sobelVKernel) for image in
                 images]
...
images_roberts = [cv2.filter2D(image , -1 , robertKernel_45) for image in
                  images]
images_roberts = [cv2.filter2D(image , -1 , robertKernel_315) for image in
                  images_roberts]
...
imagesPrewittH = [cv2.filter2D(image , -1 , prewittKernelH) for image in
                  images]
imagesPrewittV = [cv2.filter2D(image , -1 , prewittKernelV) for image in
                  images]
...
smoothedImages = [cv2.GaussianBlur(img , (5 , 5) , 2.0) for img in images]
...
for image in smoothedImages :
    sobelH = cv2.Sobel(image , -1 , 1 , 0 , ksize = 3)
    sobelV = cv2.Sobel(image , -1 , 0 , 1 , ksize = 3)
